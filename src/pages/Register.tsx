...
export default function Register() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      const result = await daftarAkun(nama, email, password);
      if (result.error) {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md flex flex-col items-center">
        <div className="w-16 h-16 bg-lime-400 rounded-2xl flex items-center justify-center text-zinc-950 rotate-3">
          <Leaf className="w-12 h-12" />
        </div>
        <h2 className="mt-6 text-center text-3xl font-display font-extrabold text-white">
          Buat Akun Baru
        </h2>
        <p className="text-center font-mono text-zinc-500 mt-2 text-sm uppercase tracking-widest">Registrasi Pelanggan</p>
      </div>

      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-zinc-900 border border-zinc-800 py-8 px-4 shadow sm:rounded-3xl sm:px-10">
          ...
          <div className="mt-6 text-center text-sm text-zinc-400">
            Sudah memiliki akun? <Link to="/login" className="text-lime-400 hover:underline">Masuk di sini</Link>
          </div>
        </div>
      </div>
    </div>
